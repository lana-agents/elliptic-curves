...
* **A span test's third verdict is not a defect count, and the binder that builds its population is
  ...
  **different** section, so it is a wrong-section row and not an unscoreable one. ⚠️ **That carries
  ...
  against 48 at `42c9b11`. ⚠️ **This commit adds four matches to the character-class reading and
  two to the published one** — `51 → 55` against `48 → 50` at `42c9b11`, `52 → 56` against
  `49 → 51` at `d559108` — because its own prose wraps two of them. The gap is a property of a ref
  and not of the seed, so it is stated as this commit's delta and not as a figure for its head.
  The published reading is a **single space**, and every one it drops is a pointer that resolves —
  three at `42c9b11`, three at `d559108`, and the two this commit adds — so the figure is short of
  a flag rather than false.
  ...
  page, so under the published reading the test stops returning the rows that quote it. **This
  commit adds 20 spans, 20 line-keyed and 18 title-matching, splitting 13 more bound and 5 more
  unbound over 18 more pairs, and moves the three verdicts by `+13 / +21 / −16`** against the
  49 / 14 / 39 above. ⚠️ **Those nine deltas are stated rather than the nine absolutes for the head
  this bullet lands at, and the difference is not presentational.** Each of the nine is identical
  spliced into `42c9b11`, into `d559108` and into `77fb54d` — the same sweep the four seed deltas
  this bullet also publishes survive — while the absolutes do not: `be1a4d5` landed between
  `d559108` and `77fb54d`, added exactly one heading citation to this page, and moved the spans,
  the line-keyed, the title-matching and the unbound by one apiece, leaving the bound, the pairs
  and all three verdicts where they were. 16 of the 39 change verdict, every one of them because
  this bullet quotes it: 13 to outside, and three to inside, which this bullet certifies by
  quoting them at themselves — it sits in `### Scope of the rules above`, so its printings of
  *"**The subject decides, not the string**"*, of *"The data the statement is about"* and of
  *"a `## Main statements` list … is one place"* land inside the section each of those three rows
  cites. ⚠️ **Under the ruled reading not one row moves**: all 48 rows the flagged test leaves
  unscoreable at `42c9b11` are unscoreable here, all 102 hold their verdict, and the difference
  between 48 and 64 is this bullet's own new pairs and nothing else. A figure that is stable under
  the reading this bullet argues for and moves by 16 under the one it argues against is the
  cheapest evidence either could have.
