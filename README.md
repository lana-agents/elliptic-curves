...
`02a1652` and at `9147113`, and 63 in 24 at `58df5a5` — the same two rows each time: in
`WeilPairing` at line 181, where a bold span closes between the index and the word `with`, and in
`WeilPairingNondegenerateN` at line 22, where the index itself is emphasised. ⚠️ **Neither reading
is wrong and they differ by two rows, so a figure over this seed owes its flag the way it owes its
sha.** What is **not** measured is
