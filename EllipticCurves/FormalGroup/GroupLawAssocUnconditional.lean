...
The `FormalGroup R` bundle itself is assembled over an **arbitrary `CommRing R`** in
`GroupLawBundleGeneral` (issue #339, the last-mile of #263): `WeierstrassCurve.formalGroup` and
`WeierstrassCurve.formalGroup_isComm` there follow from the ℚ-algebra associativity above by the
universality transfer (specialise at `R = Frac(ℤ[a₁, …, a₆])` and base-change to arbitrary `R` via
`map_formalGroupZW`, #318).  Since that general bundle subsumes the ℚ-algebra one, no separate
ℚ-algebra `formalGroup` term is provided here.
