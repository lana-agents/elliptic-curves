...
private noncomputable def exampleCurveN : Affine exampleField := ⟨0, 0, 1, 0, 0⟩
...
private lemma exampleNonsingular : exampleCurveN.Nonsingular 0 0 :=
  exampleCurveN.equation_iff_nonsingular.mp (by
    norm_num [exampleCurveN, WeierstrassCurve.Affine.equation_iff])
...
private lemma exampleTorsionThree :
    Point.some (0 : exampleField) 0 exampleNonsingular ∈ exampleCurveN.torsion 3 :=
  mem_torsion_three_some_iff'.mpr (by
    norm_num [exampleCurveN, WeierstrassCurve.Ψ₃, WeierstrassCurve.b₂, WeierstrassCurve.b₄,
      WeierstrassCurve.b₆, WeierstrassCurve.b₈])

open Classical in
/-- …and therefore `12`-torsion, by `torsion_mono` at `3 ∣ 12`. -/
private lemma exampleTorsionTwelve :
    Point.some (0 : exampleField) 0 exampleNonsingular ∈ exampleCurveN.torsion 12 :=
...
noncomputable example :
    MulSemiringAction (TorsionNMul exampleCurveN 12) exampleCurveN.FunctionField :=
  inferInstance

open Classical in
example : FaithfulSMul (TorsionNMul exampleCurveN 12) exampleCurveN.FunctionField :=
  inferInstance

open Classical in
/-- The acting group has more than one element, so the fixed field is not all of `F(W)` for a
trivial reason. -/
example : Nontrivial (TorsionNMul exampleCurveN 12) :=
...
example : Nat.card (TorsionNMul exampleCurveN 12) = 144 :=
  card_torsionNMul exampleTwo exampleThree (by norm_num) smoothTwelve

open Classical in
example : Finite (TorsionNMul exampleCurveN 12) :=
...
private lemma exampleTranscendentalTwelve :
    Transcendental exampleField ((12 : ℕ) • genericPoint (W := exampleCurveN)).xCoord :=
...
example : (mulByNEndo (W := exampleCurveN) 12 exampleTranscendentalTwelve).range
    ≤ (FixedPoints.subfield (TorsionNMul exampleCurveN 12)
      exampleCurveN.FunctionField).toSubring :=
  mulByNRange_le_fixedPoints 12 exampleTranscendentalTwelve

open Classical in
example (f : exampleCurveN.FunctionField) :
    translateAut (Point.some (0 : exampleField) 0 exampleNonsingular)
