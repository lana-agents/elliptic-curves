...
an **arbitrary** field with `(2 : F) ≠ 0`.  `weilPairingPointMuHom` restricted along the inclusion
of `E[2]`.
...
open Classical in
/-- **`e_3(S, ·) : E[3] → μ_3(F)` as a homomorphism of groups**, over an arbitrary field with
`(2 : F) ≠ 0` and `(3 : F) ≠ 0`.
