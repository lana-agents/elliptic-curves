...
* ⚠️ **`n = 5` for the BOUND only, and the reason the equality does not follow is not what four
  ...
  `(2 : F) ≠ 0` and `(n : F) ≠ 0`, `n` even included, which supersedes both *"still `3`-smooth"*
  and the odd-`n` correction this bullet used to carry.  ⚠️ **`(2 : F) ≠ 0` is the count's second
  hypothesis and this bullet used to name only the index one** (`#1137`).
