...
through surjectivity, which is blocked at composite `n` twice.  The **trace** of `ρ_{E,n}` has no
pairing-theoretic description at all and `galoisTraceTwo` is untouched by any of this.
