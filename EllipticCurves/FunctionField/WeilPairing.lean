...
(`TranslationActionThree`, `#783`) on the right.  So at `n = 3` too the gate was `hprin`, rung 5
(`#418`) — the **same** gate as at `n = 2`, and it is now discharged at both.

✅ **`hprin` is discharged at `n = 3` as well**, over an algebraically closed base field:
`EllipticCurves.FunctionField.MulByThreeFibre` supplies the fibre description and
`EllipticCurves.FunctionField.PullbackPrincipalityThree` the class computation on top of it, giving
`exists_gS_three_of_isAlgClosed`.  ⚠️ **So the asymmetry this paragraph tracked is gone, and no
paragraph below should be read as if it survived.**  What remains at `n = 3` is not a rung but a set
of corollaries — the rung-6 statements that still carry rung-5 data as a hypothesis are now
instantiable, and are not yet instantiated.  Over a *general* base field `hprin` is open at both
`n`, which is a different statement and the one `exists_gS_two`/`exists_gS_three` keep.
