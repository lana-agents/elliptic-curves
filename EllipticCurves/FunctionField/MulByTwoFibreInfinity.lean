...
* Therefore also the full fibre description `[2]∗((S) − (O)) = ∑_{R ∈ E[2]} ((P ⊕ R) − (R))` — again
  ...
  `MulByTwoFibreAffine`.
  ⚠️ `#418`'s `hprin` is not open either, at either `n`: over an algebraically closed base field
  the class-group computation on top of that description is
  `EllipticCurves.FunctionField.PullbackPrincipalityTwo` and, at `n = 3`,
  `EllipticCurves.FunctionField.PullbackPrincipalityThree`.  Over a *general* field it is still
  open at both, and that is a statement about the fibre description, not about this file.
...
* `[3]∗`.  Both steps have been transposed, in `EllipticCurves.FunctionField.MulByThreeFibre`:
  step 1 off `isCoprime_Φ_three_ΨSq_three`
  (`EllipticCurves.FunctionField.MulByThreeDegree`), step 2 against
  `EllipticCurves.FunctionField.MulByThreeRamification`'s `∑ e_p = 9` in place of `#763`'s `4`.
  ⚠️ The `n = 3` count is *not* reached the way the `4` here is; see
  `EllipticCurves.FunctionField.MulByTwoFibreAffine`'s scope note.
