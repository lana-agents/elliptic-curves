...
property of a bilinear map"* — and `EllipticCurves.FunctionField.WeilPairingFunctionThree` (`#925`)
was written to remove it, as `WeilPairingFunctionTwo` (`#922`) had at `n = 2`.  That file defines
`weilPairingThree : E[3] → E[3] → μ_3(F)` as a genuine function of two torsion points, bundles it as
`weilPairingThreeHom`, and states `ker_weilPairingThreeHom h2 h3 = ⊥`, which is precisely the
property of a bilinear map.  Its proof reaches `eq_zero_of_forall_weilPairingElt_eq_one_three` below
in three steps and consumes nothing else — `ker_weilPairingThreeHom`, then
`eq_zero_of_forall_weilPairingThree_eq_one`, then `eq_zero_of_forall_weilPairingEltThree_eq_one`,
which applies it to the chosen root.
...
below**: `divisor W f = (3 : ℤ) • pointDivisorAff W S`, with `S : W.Point`, is the one shape that
covers `O` and the affine points at once, which is why `#925` could take it as its root predicate
verbatim.
