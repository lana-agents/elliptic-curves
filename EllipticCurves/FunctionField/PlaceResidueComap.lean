...
* `WeierstrassCurve.Affine.CoordinateRing.residueDegreeTwo` and
  ...
  from nothing but `residueDegreeProj W none ≠ 0`.

## What is *not* here: the fundamental identity, and the route decision it was blocked on

**Nothing below proves `∑_{p ↦ q} e_p · f_p = 4`.**  The sibling issue asked for a *written route
decision, with evidence, before any theorem*, and the decision is that the identity does not close
in one session — not for want of effort, but because it rests on three named prerequisites that are
absent from this tree.  Recording them is the deliverable; guessing at a half-built proof is not.
...
* **(c-i) `IsDiscreteValuationRing (placeOf W q)` is not available.**  It is explicitly disclaimed
  in `PlaceResidueField.lean` and in `ValuationAtInfinity.lean`, whose docstring says discreteness
  at infinity is proved only in the doubled form `exists_zpow_eq`.  Route (c) wants `R` at least
  Noetherian, and the natural `R` is the valuation ring of `q`.  This one is a *small* issue rather
  than a research problem: `exists_divisorProj_eq_one` already gives a uniformizer at every place —
  i.e. the value group surjects onto `ℤ` — and `mem_placeOf_iff_divisorProj_nonneg` turns ideal
  membership into an order inequality, which is the whole of the argument that every ideal is
  generated by a power of a uniformizer.
* **(c-ii) `Module.Finite` for the integral closure needs separability, which the tree never
  states.**  Mathlib reaches module-finiteness of an integral closure through
  `IsIntegralClosure.finite` (`DedekindDomain/IntegralClosure.lean`), which wants
  `[IsIntegrallyClosed R] [IsNoetherianRing R] [IsFractionRing R L] [Algebra.IsSeparable L F(W)]`.
  The `IsNoetherianRing R` is (c-i) again.  Searching this tree for `separable` returns four hits,
  in
  `TateModule/GaloisAction.lean`, `TateModule/Image.lean` and `Torsion/ThreeTorsionStructure.lean`;
  none is about `F(W) / [2]∗F(W)`.  **In characteristic zero it is free**, and that is the cheap
  first version to ship: `Algebra.IsSeparable.of_integral` (`Mathlib/FieldTheory/Separable.lean`) is
  an instance under `[Algebra.IsIntegral F K] [CharZero F]`, and `module_finite_mulByTwoRange`
  already supplies the integrality.  Flatness, by contrast, is *not* a blocker: over a discrete
  valuation ring torsion-free implies flat, and a subring of `F(W)` is torsion-free.
* **(c-iii) the fibre dictionary is missing.**  `p.primesOver S` has to be matched with
  `(comapProjPointTwo h2) ⁻¹' {q}`.  `finite_comapProjPointTwo_preimage_singleton` (`#675`) makes
  the sum *statable*, but nothing identifies the two index sets, and the `none` branch of
  `ProjPoint W = Option (HeightOneSpectrum W.CoordinateRing)` is not a prime of any ring — the
  asymmetry `placeOf` was built to dissolve.  Expect this to be the largest of the three.
...
Also not here: `ramificationIdxTwo` at an affine place (nothing anywhere computes it), any
comparison of `residueDegreeProj` with `degPt` (`DivisorDegree.lean`), which is a relative ideal
norm to `F[X]` and not a residue-field degree, and `#E[n] = n²`, whose link to the field degree runs
through the separability argument of (c-ii).
...
`F(W) / [2]∗F(W)` at a place.  Their fundamental identity `∑_{p ↦ q} e_p · f_p = 4` is **not**
proved anywhere; see the module docstring for the route decision. -/
...
`f_p = 1` everywhere and the fundamental identity would read `∑_{p ↦ q} e_p = 4`. -/
