...
⚠️ **Qualified, not retired: *"general `n`"* is out of scope of THIS file and stays out of it, but
the general-`n` `μ_n(F)` layer now exists** —
`EllipticCurves.FunctionField.WeilPairingNondegenerateN` (`#1844`) states this file's three
headline shapes at every `n` with `(2 : F) ≠ 0` and `((n : ℤ) : F) ≠ 0` over `F̄`, off the same
descent `weilPairingMu_eq_one_iff` used here and off the general-`n` `hpow` producer
