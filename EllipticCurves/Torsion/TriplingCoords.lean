...
the point at infinity; above that, `EllipticCurves.FunctionField.MulByThreeRamification` supplies
the `n = 3` analogue of `#763`'s ramification-index arithmetic, and
`EllipticCurves.FunctionField.MulByThreeFibre` the fibre description itself, which consumes this
file for the one step that needs the tripling coordinates.  What is left above them is the
class-group computation, the mirror of
