...
* `#E[p] = p²` for a prime `p ≥ 5`.  ⚠️ This bullet used to read *"`#E[p] ≤ p²` …, which needs the
  ...
  `card_torsion_le_sq` (`EllipticCurves.Torsion.XSupport`) at every index over a field with
  `(2 : F) ≠ 0` and `(n : F) ≠ 0`, and the coordinate formula is proved at every index with
  `(2 : F) ≠ 0` (`hasXCoordFormula_of_two_ne_zero`, `EllipticCurves.Torsion.NsmulOrder`).
  ⚠️ The `≥` half is no longer open either: `card_torsion_eq_sq_of_odd`
  (`EllipticCurves.Torsion.OmegaChordSum`) proves the equality at every odd `n` with
  `(n : F) ≠ 0`, over a field with `(2 : F) ≠ 0`, and `EllipticCurves.Torsion.PrimaryTowerOdd`
  runs the tower on it.  ⚠️ **Both hypotheses, and this clause used to name neither** (`#1137`).
  What this bullet still records correctly is that none of that happens here;
