...
⚠️ **That input is proved too, at every index with `(2 : F) ≠ 0`** —
`hasXCoordFormula_of_two_ne_zero` (`EllipticCurves.Torsion.NsmulOrder`, which imports this file) —
so `#251` is closed and nothing here names an open gate.
