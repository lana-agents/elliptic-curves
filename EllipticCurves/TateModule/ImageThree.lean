...
* **General odd `ℓ ≥ 5` stays out.** `EllipticCurves.TateModule.PrimaryImage` is already stated at
  ...
  crux"*, and all three clauses are wrong: `[5]`-surjectivity holds at every nonzero index with
  `(2 : F) ≠ 0` (`nsmul_surjective_of_two_ne_zero`, `EllipticCurves.Torsion.TwoTorsionOrder`); the
  coordinate formula is proved at every index with `(2 : F) ≠ 0` (`hasXCoordFormula_of_two_ne_zero`,
  ...
  attribution this bullet used to carry.  ⚠️ **`(2 : F) ≠ 0` is the second hypothesis and this
  bullet used to name only the index one** (`#1137`); it is a hypothesis of the count and of all 22
  ...
  **`EllipticCurves.TateModule.ImageGeneral` supplies it at every prime `ℓ` with `(2 : F) ≠ 0` and
  `(ℓ : F) ≠ 0`** — so *"separate work and is not done here"* is discharged rather than owed. ⚠️ The
  clause that followed — *"`ℓ ≥ 5` gains the generic file and nothing else"* — was the right
  diagnosis, and it is what has been answered: what was missing was a basis to feed the generic
  file, not a theorem in it.  ⚠️ `(ℓ : F) ≠ 0` is sharp: at `ℓ = char F` the conclusion is
  **false**, not open — `E[ℓ]` is `0` or `ℤ/ℓℤ`, so `T_ℓE` has rank `0` or `1`.
