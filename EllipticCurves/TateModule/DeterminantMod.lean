...
`EllipticCurves.TateModule.Kernel`, live here because `private` hides a *name*, not an *instance*,
so it takes part in resolution in every module downstream of the one that declares it.
