...
* ⚠️ **`ℓ ≥ 5` gains nothing from this file being generic.** Its hypothesis
  `Nonempty (T_ℓE ≃ₗ ℤ_[ℓ]²)` is gated at `ℓ ≥ 5` on `[ℓ]`-surjectivity and `#E[ℓ^k]`, both of which
  need the general coordinate formula `x(nP) = Φₙ/ΨSqₙ`, i.e. the `ωₙ` crux. What being generic
  buys is that when that gate is paid, the `ℓ = 5` file is again a list of instantiations and no
