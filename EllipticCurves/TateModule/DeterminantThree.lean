...
* **General odd `ℓ ≥ 5` stays out.** `EllipticCurves.TateModule.PrimaryDeterminant` is already
  ...
  **`EllipticCurves.TateModule.DeterminantGeneral` supplies it at every prime `ℓ` with `(2 : F) ≠ 0`
  and `(ℓ : F) ≠ 0`** — so *"separate work and is not done here"* is discharged rather than owed.
  ⚠️ `(ℓ : F) ≠ 0` is sharp: at `ℓ = char F` the conclusion is **false**, not open — `E[ℓ]` is `0`
  or `ℤ/ℓℤ`, so `T_ℓE` has rank `0` or `1`.
